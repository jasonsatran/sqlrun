...
class ProcessReport:
    ...
    def get_report(self):
        ...
        for process in self._processes:
            ...
            if process_result == None:
                report_row.append(template.format(
                    str(i), process.description, "No Result"))
            else:
                report_row.append(template.format(str(i),
                                                  process.description,
                                                  ProcessReport.format_second(
                                                      process_result.running_time),
                                                  ProcessReport.format_percent(
                                                      process_result.percent_of_total())
                                                  )
                                  )
            i += 1
    ...
    def get_report_header(self):
        ...
        header_len = len(header)
        header_data = template.format(str(start_time), str(end_time), str(
            processes_len), ProcessReport.local_time(start_time), ProcessReport.local_time(end_time))
